...
from src.evaluation.evaluate_merged_map import (
    RenderFrames,
    merge_submaps,
    refine_global_map,
)
...
class Evaluator:
    ...
    def __init__(
        self, checkpoint_path, config_path, config=None, save_render=False
    ) -> None:
        if config is None:
        ...
        self.estimated_c2w = torch2np(
            torch.load(
                self.checkpoint_path / "estimated_c2w.ckpt", map_location=self.device
            )
        )
    ...
    def run_rendering_eval(self):
        ...
        opt_settings = OptimizationParams(
        ...
        for submap_path in tqdm(submaps_paths):
            ...
            for keyframe_id in submap["submap_keyframes"]:
                ...
                psnr_value = (-10.0 * torch.log10(mse_loss)).item()
        ...
        save_dict_to_json(
            metrics, "rendering_metrics.json", directory=self.checkpoint_path
        )
        ...
        plt.tight_layout()
        plt.savefig(str(self.checkpoint_path / "rendering_metrics.png"), dpi=300)
        print(metrics)
    ...
    def run_reconstruction_eval(self):
        ...
        for submap_path in tqdm(submaps_paths):
            ...
            for keyframe_id in submap["submap_keyframes"]:
                ...
                rendered_color = (
                    torch2np(rendered_color.permute(1, 2, 0)) * 255
                ).astype(np.uint8)
    ...
    def run_global_map_eval(self):
        ...
        training_frames = RenderFrames(
            self.dataset, self.estimated_c2w, self.height, self.width, self.fx, self.fy
        )
        ...
        refined_merged_gaussian_model = refine_global_map(
            merged_cloud, training_frames, 10000
        )
        ply_path = (
            self.checkpoint_path / f"{self.config['data']['scene_name']}_global_map.ply"
        )
        ...
        test_set = get_dataset(eval_config["dataset_name"])(
            {**eval_config["data"], **eval_config["cam"]}
        )
        test_poses = torch.stack(
            [torch.from_numpy(test_set[i][3]) for i in range(len(test_set))], dim=0
        )
        test_frames = RenderFrames(
            test_set, test_poses, self.height, self.width, self.fx, self.fy
        )
        ...
        for i in tqdm(range(len(test_set))):
            ...
            render_dict = render_gaussian_model(
                refined_merged_gaussian_model, render_settings
            )
            rendered_color, _ = (
                render_dict["color"].permute(1, 2, 0),
                render_dict["depth"],
            )
            rendered_color = torch.clip(rendered_color, 0, 1)
            save_image(
                rendered_color.permute(2, 0, 1),
                self.checkpoint_path / f"nvs_eval/{i:04d}.jpg",
            )
