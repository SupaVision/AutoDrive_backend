...
class RGBDImage:
    def __init__(self, rgb_image: np.ndarray,
        ...
        if not isinstance(scale, float) and scale is not None:
            raise TypeError(f"Expected scale to be of type float; got {type(scale)}")
    ...
    def _preprocess_color(self, rgb_image: np.ndarray, *, normalize_color: bool = True,
        ...
        Args:
            rgb_image (np.ndarray): Raw input rgb image
        Reruns:
    ...
    def _preprocess_depth(self, depth_image: np.ndarray,
        ...
        Args:
            ...
            self.scale (float): scale to convert depth values to meters
        Returns:
        ...
        return torch.from_numpy(depth / self._scale).to(self._device)
